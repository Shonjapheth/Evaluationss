...
class Action
{
	...
	function save_user()
	{
		...
		$check = $conn->query("SELECT * FROM users where email ='$email' " . (!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if ($check > 0) {
			return 2;
			exit;
		}
	...
	function update_user()
	{
		...
		$check = $conn->query("SELECT * FROM {$type[$_SESSION['login_type']]} where email ='$email' " . (!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if ($check > 0) {
			return 2;
			exit;
		}
		...
		if (!empty($password))
			$data .= " ,password=md5('$password') ";
		if (empty($id)) {
			$save = $conn->query("INSERT INTO {$type[$_SESSION['login_type']]} set $data");
		} else {
			echo "UPDATE {$type[$_SESSION['login_type']]} set $data where id = $id";
			$save = $conn->query("UPDATE {$type[$_SESSION['login_type']]} set $data where id = $id");
		}
	...
	function save_system_settings()
	{
		...
		if ($chk->num_rows > 0) {
			$save = $conn->query("UPDATE system_settings set $data where id =" . $chk->fetch(PDO::FETCH_ASSOC)['id']);
		} else {
	...
	function save_subject()
	{
		...
		$chk = $conn->query("SELECT * FROM subject_list where code = '$code' and id != '{$id}' ")->num_rows;
		if ($chk > 0) {
			return 2;
		}
		if (empty($id)) {
	...
	{
		extract($_POST);
		$delete = $conn->query("DELETE FROM subject_list where id = $id");
		if ($delete) {
			return 1;
		}
	}
	function save_class()
	{
		...
		$chk = $conn->query("SELECT * FROM class_list where (" . str_replace(",", 'and', $data) . ") and id != '{$id}' ")->num_rows;
		if ($chk > 0) {
			return 2;
		}
		if (isset($user_ids)) {
	...
	{
		extract($_POST);
		$delete = $conn->query("DELETE FROM class_list where id = $id");
		if ($delete) {
			return 1;
		}
	}
	function save_academic()
	{
		...
		$chk = $conn->query("SELECT * FROM academic_list where (" . str_replace(",", 'and', $data) . ") and id != '{$id}' ")->num_rows;
		if ($chk > 0) {
			return 2;
		}
		$hasDefault = $conn->query("SELECT * FROM academic_list where is_default = 1")->num_rows;
		if ($hasDefault == 0) {
			$data .= " , is_default = 1 ";
		}
		if (empty($id)) {
	...
	{
		extract($_POST);
		$delete = $conn->query("DELETE FROM academic_list where id = $id");
		if ($delete) {
			return 1;
		}
	}
	...
	{
		extract($_POST);
		$update = $conn->query("UPDATE academic_list set is_default = 0");
		$update1 = $conn->query("UPDATE academic_list set is_default = 1 where id = $id");
		$qry = $conn->query("SELECT * FROM academic_list where id = $id")->fetch(PDO::FETCH_ASSOC);
		if ($update && $update1) {
			foreach ($qry as $k => $v) {
				if (!is_numeric($k))
					$_SESSION['academic'][$k] = $v;
	...
	function save_criteria()
	{
		...
		$chk = $conn->query("SELECT * FROM criteria_list where (" . str_replace(",", 'and', $data) . ") and id != '{$id}' ")->num_rows;
		if ($chk > 0) {
			return 2;
		}
	...
	{
		extract($_POST);
		$delete = $conn->query("DELETE FROM criteria_list where id = $id");
		if ($delete) {
			return 1;
		}
	}
	function save_criteria_order()
	{
		...
		foreach ($criteria_id as $k => $v) {
			$update[] = $conn->query("UPDATE criteria_list set order_by = $k where id = $v");
		}
	...
	function save_question_order()
	{
		...
		$data = "";
		foreach ($qid as $k => $v) {
			$update[] = $conn->query("UPDATE question_list set order_by = $k where id = $v");
		}
		if (isset($update) && count($update)) {
			return 1;
		}
	...
	function save_faculty()
	{
		...
		$check = $conn->query("SELECT * FROM faculty_list where email ='$email' " . (!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if ($check > 0) {
			return 2;
			exit;
		}
		$check = $conn->query("SELECT * FROM faculty_list where school_id ='$school_id' " . (!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if ($check > 0) {
			return 3;
			exit;
		}
	...
	{
		extract($_POST);
		$delete = $conn->query("DELETE FROM faculty_list where id = " . $id);
		if ($delete)
			return 1;
	}
	function save_student()
	{
		...
		$check = $conn->query("SELECT * FROM student_list where email ='$email' " . (!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if ($check > 0) {
			return 2;
			exit;
		}
	...
	function save_progress()
	{
		...
		if (!isset($is_complete))
			$data .= ", is_complete=0 ";
		if (empty($id)) {
			$save = $conn->query("INSERT INTO task_progress set $data");
		} else {
			$save = $conn->query("UPDATE task_progress set $data where id = $id");
		}
		if ($save) {
			if (!isset($is_complete))
				$conn->query("UPDATE task_list set status = 1 where id = $task_id ");
			else
				$conn->query("UPDATE task_list set status = 2 where id = $task_id ");
			return 1;
		}
	}
	...
	{
		extract($_POST);
		$delete = $conn->query("DELETE FROM task_progress where id = $id");
		if ($delete) {
			return 1;
		}
	}
	function save_restriction()
	{
		...
		if (!empty($filtered))
			$conn->query("DELETE FROM restriction_list where id not in ($filtered) and academic_id = $academic_id");
		else
			$conn->query("DELETE FROM restriction_list where  academic_id = $academic_id");
		foreach ($rid as $k => $v) {
			...
			if (empty($v)) {
				...
				$save[] = $conn->query("UPDATE restriction_list set $data where id = $v ");
			}
		}
		return 1;
	}
	function save_evaluation()
	{
		...
		if ($save) {
			...
			foreach ($qid as $k => $v) {
				...
				$data .= ", question_id = $v ";
				$data .= ", rate = {$rate[$v]} ";
				$ins[] = $conn->query("INSERT INTO evaluation_answers set $data ");
			}
			if (isset($ins))
				return 1;
		}
	...
	function get_report()
	{
		extract($_POST);
		$data = array();
		$get = $conn->query("SELECT * FROM evaluation_answers where evaluation_id in (SELECT evaluation_id FROM evaluation_list where academic_id = {$_SESSION['academic']['id']} and faculty_id = $faculty_id and subject_id = $subject_id and class_id = $class_id ) ");
		$answered = $conn->query("SELECT * FROM evaluation_list where academic_id = {$_SESSION['academic']['id']} and faculty_id = $faculty_id and subject_id = $subject_id and class_id = $class_id");
		$rate = array();
		while ($row = $get->fetch_assoc()) {
